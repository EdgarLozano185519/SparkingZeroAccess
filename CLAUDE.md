...
- **Mod framework:** UE4SS v3.0.1 (Lua mod). Experimental UE4SS builds kill this game a few seconds after start even with the mod disabled (tested 2026-09-13); `helpers\Switch-UE4SS.ps1 -Build stable` restores 3.0.1 from `build\backup`. All mod work runs on the game thread through `game_thread.lua` (`GT.Every` / `GT.After` / `GT.OnKey`, timer + ExecuteInGameThread on 3.0.1); `LoopAsync`, `ExecuteWithDelay`, `ExecuteInGameThread`, `RegisterKeyBind` are only allowed in game_thread.lua (luacheck enforces). No `RegisterHook` callbacks in this mod (one on ClientRestart crashed the game at startup). Reason for all this: async polling raced widget destruction (World Tournament crash)
- **Object lookups:** `objects.lua` replaces the `FindAllOf` / `FindFirstOf` globals with a per-class cache (walks are 28–50 ms each on 3.0.1; at most one per tick, none in the first 5 s or while battle is busy). Never walk "Widget" or "Actor" (killed the game). Widget children are also reachable as properties (`widget.Text_EventTitle`)
- **Speech:** native plugin `speech_plugin\SparkingZeroSpeech.asi` (C, MSVC; `speech_plugin\build.ps1 -Deploy`) loaded by the bypass's Ultimate ASI Loader from `Win64\plugins`, serving the named pipe `\\.\pipe\SparkingZeroSpeech` that `speech.lua` writes to, speaking via UniversalSpeech (NVDA/JAWS/SAPI). No NVDA add-on, no UE4SS dependency (user decision). Plugin log: `Win64\plugins\SparkingZeroSpeech.log`
- **Test launches without the user:** `helpers\Launch-Game.ps1` (start via Steam, watch log, stop) and `helpers\Drive-Game.ps1` (also sends keys). Both print the exit code (3 = Unreal's own fatal-error exit, the former "silent exit") and new dumps. The speech plugin's crash catcher (vectored exception handler) logs every game-thread crash with a stack walk in `Win64\plugins\SparkingZeroSpeech.log` and writes `Win64\plugins\AE_crash_*.dmp`
- **Object lifetime:** UE4SS 3.0.1 `IsValid()` reads the object's own memory, so cached references are only safe until the next garbage collection. `objects.lua` detects every GC (sentinel object) and calls `Objects.OnFlush` listeners; any module that keeps a UObject reference across ticks must drop it there (see `Battle.InvalidateRefs`). Walks (`FindAllOf`) can crash while the game streams assets: keep them rare
...
- **Game thread timing:** `UE4SS.log` gets `[AE] Slow game thread task` lines (>12 ms), a `[AE] Game thread 60s:` summary and an `[AE] Objects:` summary every minute, plus one `[AE] Objects: walked <class>` line per walk. Check them after every play test; slow tasks stutter the game now that polling is on the game thread
...
Crash dumps: `crash_*.dmp` in `Win64` are written by UE4SS's crash handler (its own threads only); `plugins\AE_crash_*.dmp` by the speech plugin's crash catcher (game thread too). No debugger is installed; read them with `experiments\mdump.py` and `experiments\threadroots.py` (Python stdlib, on this branch).
