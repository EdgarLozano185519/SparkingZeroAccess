...
- **Test launches without the user:** `helpers\Launch-Game.ps1` (start via Steam, watch log, stop) and `helpers\Drive-Game.ps1` (also sends keys). Game-thread crashes exit silently: no dump, no Windows event, log stops
