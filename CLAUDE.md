...
- **Mod framework:** UE4SS experimental build (UE4SS_v3.0.1-1133-gb4cefa18, Lua mod) since 2026-09-12, installed with `helpers\Switch-UE4SS.ps1 -Build experimental` (`-Build stable` restores 3.0.1 from `build\backup`). All mod work runs on the game thread through `game_thread.lua` (`GT.Every` / `GT.After` / `GT.OnKey`); `LoopAsync`, `ExecuteWithDelay`, `ExecuteInGameThread`, `RegisterKeyBind` are only allowed in game_thread.lua (luacheck enforces). Reason: async polling raced widget destruction (World Tournament crash)
- **Speech:** named pipe `\\.\pipe\SparkingZeroAccess` (`speech.lua`, plain Lua io) served by the NVDA add-on in `nvda-addon\` (build: `helpers\Build-NvdaAddon.ps1`; installed copy in `%APPDATA%\nvda\addons\SparkingZeroAccess`). speech_bridge.dll is retired (Lua C modules can't work on newer UE4SS). Offline test: `experiments\Test-SpeechPipe.ps1`
...
- **Game thread timing:** `UE4SS.log` gets `[AE] Slow game thread task` lines (>12 ms) and a `[AE] Game thread 60s:` summary every minute. Check them after every play test; slow tasks stutter the game now that polling is on the game thread
